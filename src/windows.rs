...
use winapi::um::wingdi::GetICMProfileA;
use winapi::um::winreg::{
    RegCloseKey, RegEnumKeyExA, RegOpenKeyExA, RegQueryValueExA, HKEY_LOCAL_MACHINE,
};
use winapi::um::winuser::{EnumDisplayMonitors, GetMonitorInfoA, MONITORINFO, MONITORINFOEXA};
...
fn get_color_directory() -> Result<PathBuf, ProfileError> {
    ...
    let color_dir = PathBuf::from(windows_dir)
        .join("System32")
        .join("spool")
        .join("drivers")
        .join("color");
...
fn get_monitor_profile(_monitor: HMONITOR) -> Result<String, ProfileError> {
    ...
    let result = unsafe { GetICMProfileA(hdc, &mut size, buffer.as_mut_ptr() as *mut i8) };
...
fn read_profile_file(profile_path: &PathBuf) -> Result<Vec<u8>, ProfileError> {
    std::fs::read(profile_path).map_err(|e| ProfileError::IoError(e.to_string()))
}
...
fn query_registry_for_profiles() -> Result<Vec<String>, ProfileError> {
    ...
    loop {
        ...
        if !key_name_str.is_empty() {
            if let Ok(profile) = query_display_profile_from_registry(
                hkey as winapi::shared::ntdef::HANDLE,
                key_name_str,
            ) {
...
fn query_display_profile_from_registry(
    parent_key: winapi::shared::ntdef::HANDLE,
    display_name: &str,
...
fn handle_windows_permissions_error(error: &std::io::Error) -> ProfileError {
    match error.kind() {
        std::io::ErrorKind::PermissionDenied => ProfileError::SystemError(
            "Access denied. Administrator privileges may be required to access color profiles."
                .to_string(),
        ),
        std::io::ErrorKind::NotFound => {
            ProfileError::ProfileNotAvailable("Color profile file not found".to_string())
        }
                    ...
                    match std::fs::read(&profile_path) {
                        Ok(data) => {
                            let (name, description, color_space) = parse_icc_header(&data)
                                .unwrap_or_else(|_| {
                                    (profile_name.clone(), None, ColorSpace::Unknown)
                                });
            ...
            if profile_path.exists() {
                match std::fs::read(&profile_path) {
                    Ok(data) => {
                        let (name, description, color_space) = parse_icc_header(&data)
                            .unwrap_or_else(|_| {
                                (profile_name.to_string(), None, ColorSpace::Unknown)
                            });
                    ...
                    Ok(profiles) => {
                        for profile_name in profiles {
                            ...
                            if profile_path.exists() {
                                match std::fs::read(&profile_path) {
                                    Ok(data) => {
                                        let (name, description, color_space) =
                                            parse_icc_header(&data).unwrap_or_else(|_| {
                                                (profile_name.clone(), None, ColorSpace::Unknown)
                                            });
