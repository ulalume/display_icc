...
fn test_cli_info_command() {
    ...
    if output.status.success() {
        ...
        assert!(stdout.contains("Display:"), "Should show display information");
        ...
        if stdout.contains("Profile:") {
            assert!(stdout.contains("Color space:"), "Should show color space if profile exists");
        }
    } else {
        let stderr = String::from_utf8_lossy(&output.stderr);
        println!("CLI info failed (may be expected on some systems): {}", stderr);
...
fn test_cli_info_json_format() {
    ...
    if output.status.success() {
        ...
        assert!(display.get("is_primary").is_some(), "Display should have is_primary");
        
        // Profile may or may not be present
        if json.get("profile").is_some() {
            let profile = &json["profile"];
            assert!(profile.get("name").is_some(), "Profile should have name");
            assert!(profile.get("color_space").is_some(), "Profile should have color_space");
        }
    } else {
        let stderr = String::from_utf8_lossy(&output.stderr);
...
fn test_cli_list_command() {
    ...
    if output.status.success() {
        ...
        assert!(stdout.contains("Available displays:") || stdout.contains("Display:"), 
...
fn test_cli_list_json_format() {
    ...
    if output.status.success() {
        ...
        let displays = json["displays"].as_array()
        ...
        if !displays.is_empty() {
            ...
            assert!(first_display.get("name").is_some(), "Display should have name");
            assert!(first_display.get("is_primary").is_some(), "Display should have is_primary");
        }
    } else {
        let stderr = String::from_utf8_lossy(&output.stderr);
...
fn test_cli_list_verbose() {
    ...
    if output.status.success() {
        ...
        assert!(stdout.contains("Display:") || stdout.contains("Available displays:"), 
               "Should show display information");
    } else {
        let stderr = String::from_utf8_lossy(&output.stderr);
...
fn test_cli_export_command() {
    ...
    if output.status.success() {
        ...
        assert!(stdout.contains("Exported") || stdout.contains("Profile size:"), 
...
fn test_cli_export_json_format() {
    ...
    if output.status.success() {
        ...
        assert!(json.get("display").is_some(), "Should have display object");
        assert!(json.get("output_file").is_some(), "Should have output_file field");
        assert!(json.get("size_bytes").is_some(), "Should have size_bytes field");
        
        // Verify the file was actually created
        let output_file = json["output_file"].as_str()
...
fn test_cli_header_command() {
    ...
    if output.status.success() {
        ...
        assert!(stdout.contains("ICC Profile Header") || stdout.contains("Profile size:"), 
               "Should show ICC header information");
        
        if stdout.contains("Version:") {
            assert!(stdout.contains("Device class:"), "Should show device class");
            assert!(stdout.contains("Data color space:"), "Should show color space");
        }
    } else {
        let stderr = String::from_utf8_lossy(&output.stderr);
...
fn test_cli_header_json_format() {
    ...
    if output.status.success() {
        ...
        assert!(json.get("display").is_some(), "Should have display object");
        assert!(json.get("icc_header").is_some(), "Should have icc_header object");
        
        let header = &json["icc_header"];
        assert!(header.get("profile_size").is_some(), "Header should have profile_size");
        assert!(header.get("version").is_some(), "Header should have version");
        assert!(header.get("device_class").is_some(), "Header should have device_class");
    } else {
        let stderr = String::from_utf8_lossy(&output.stderr);
...
fn test_cli_help() {
    ...
    assert!(stdout.contains("COMMANDS") || stdout.contains("Commands"), "Should show commands");
...
fn test_cli_version() {
    ...
    assert!(stdout.contains("display_icc") || stdout.contains("0.1.0"), 
...
fn test_cli_invalid_command() {
    ...
    assert!(!stderr.is_empty(), "Should show error message for invalid command");
}
...
fn test_cli_output_consistency() {
    ...
    if output1.status.success() && output2.status.success() {
        ...
        if let (Ok(json1), Ok(json2)) = (
            ...
            assert_eq!(json1.get("display"), json2.get("display"), 
            ...
            if json1.get("profile").is_some() && json2.get("profile").is_some() {
                assert_eq!(json1.get("profile"), json2.get("profile"), 
