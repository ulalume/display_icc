...
use display_icc::{
    ...
    get_primary_display_profile, get_primary_display_profile_data, parse_icc_header,
    ColorSpace, Platform, ProfileConfig, ProfileError,
};
...
fn test_create_provider_default() {
    ...
    assert!(provider.is_ok(), "Should create provider on supported platform");
}
...
fn test_create_provider_with_config() {
    ...
    assert!(provider.is_ok(), "Should create provider with custom config");
}
...
fn test_get_primary_display_integration() {
    let provider = create_provider().expect("Should create provider");
    let primary = provider.get_primary_display().expect("Should get primary display");
    
    assert!(primary.is_primary, "Primary display should be marked as primary");
    assert!(!primary.id.is_empty(), "Primary display ID should not be empty");
    assert!(!primary.name.is_empty(), "Primary display name should not be empty");
}
...
fn test_get_profile_data_integration() {
    ...
    match provider.get_profile_data(&primary) {
        Ok(data) => {
            ...
            assert!(data.len() >= 128, "Profile data should be at least 128 bytes (ICC header)");
    ...
    match get_all_display_profiles() {
        Ok(profiles) => {
            ...
            for (display, profile) in &profiles {
                ...
                assert!(display_ids.insert(display.id.clone()), 
                       "Display IDs should be unique");
                
                assert!(!display.name.is_empty(), "Display name should not be empty");
                assert!(!profile.name.is_empty(), "Profile name should not be empty");
                
                println!("  {}: {} ({})", display.name, profile.name, profile.color_space);
            }
            ...
            if !all_displays.is_empty() {
                ...
                assert!(profiles.len() <= all_displays.len(), 
    ...
    match get_primary_display_profile_data() {
        Ok(data) => {
            ...
            if data.len() >= 4 {
                ...
                assert!(profile_size >= 128, "Profile size should be at least 128 bytes");
                assert!(profile_size <= 10_000_000, "Profile size should be reasonable (< 10MB)");
            }
        }
        Err(ProfileError::ProfileNotAvailable(_)) => {
            println!("No profile data available for primary display (this is valid)");
...
fn test_different_configurations() {
    ...
    for (i, config) in configs.iter().enumerate() {
        ...
        assert!(!displays.is_empty(), "Should have displays with config {}", i);
        
        let primary = provider.get_primary_display()
            .expect("Should get primary display with any config");
        
        assert!(primary.is_primary, "Primary should be marked as primary with config {}", i);
    }
}
...
mod macos_tests {
    ...
    fn test_macos_specific_behavior() {
        ...
        let displays = provider.get_displays().expect("Should get displays on macOS");
        
        // macOS should always have at least the main display
        assert!(!displays.is_empty(), "macOS should have at least one display");
        
        // Test that display IDs are numeric (CGDirectDisplayID)
        for display in &displays {
            // macOS display IDs should be parseable as numbers
            display.id.parse::<u32>()
    ...
    fn test_linux_specific_behavior() {
        ...
        for config in &configs {
            ...
            match provider.get_displays() {
                Ok(displays) => {
                    println!("Linux config {:?}: found {} displays", config, displays.len());
                }
                Err(e) => {
...
mod windows_tests {
    ...
    fn test_windows_specific_behavior() {
        ...
        let displays = provider.get_displays().expect("Should get displays on Windows");
        
        // Windows should always have at least one display
        assert!(!displays.is_empty(), "Windows should have at least one display");
        
        // Test that we can get profiles (Windows may or may not have profiles assigned)
        for display in &displays {
            match provider.get_profile(display) {
                Ok(profile) => {
                    println!("Windows display '{}' has profile '{}'", display.name, profile.name);
                ...
                Err(ProfileError::ProfileNotAvailable(_)) => {
                    println!("Windows display '{}' has no profile (this is valid)", display.name);
                }
                Err(e) => {
...
fn test_rapid_calls() {
    ...
    for i in 0..10 {
        ...
        assert!(!displays.is_empty(), "Should have displays on iteration {}", i);
        
        let primary = provider.get_primary_display()
            .expect(&format!("Should get primary display on iteration {}", i));
        
        assert!(primary.is_primary, "Primary should be primary on iteration {}", i);
    }
}
